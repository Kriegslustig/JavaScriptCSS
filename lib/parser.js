...
function Parser () { return {
  ...
  include: function (somePath) {
    ...
    return fileHandler.getJsheets(somePath).map(function (fileName) {
      var hooksStash = [_.clone(self.context.onEOF), _.clone(self.context.onAfterParse)]
      self.context.onEOF = []
      self.context.onAfterParse = []
      var fileString = self.parse(fileHandler.read(fileName)) || ''
      self.context.onEOF = hooksStash[0]
      self.context.onAfterParse = hooksStash[1]
      return fileString
    }).join('\n')
  },
  ...
  getHooks: function (hook) {
    ...
    return _.clone(this.context.hooks[hook])
  },

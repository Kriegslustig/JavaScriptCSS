...
module.exports = {
  ...
  /*
   * Utilities are objects availible within the JS part of a jsheet file that live in global space
   */
  utilities: {
    _: _
  },
  ...
  exec: function (javaScriptString) {
    ...
    try {
      ...
      console.log('Exception in your jsheet')
  ...
  split: function (jsheetString) {
    ...
    */
    ...
    if(!jsheetString) return false
    cssRegexRes = cssRegex.exec(jsheetString)
    if(!cssRegexRes) return ['js', jsheetString, false]
    returnLang = jsheetString.indexOf(cssRegexRes[0]) === 0 ? 'css' : 'js'
    return [returnLang].concat(this.splitAroundSubstring(cssRegexRes[0], jsheetString))
  },
  ...
  makeIncludes: function (jsheetString) {
    var self = this
    var includeRegex = /include\((\'|\")([^\)]*)\1\)/gm
    jsheetString = jsheetString.replace(includeRegex, function (match, g1, g2) {
      return self.include(g2)
    })
    return jsheetString
  },

  parse: function (jsheetString) {
    var splitJcss = []
    var buffer = ''
    var parsedCss = ''
    if(typeof jsheetString != 'string') return false
    jsheetString = this.makeIncludes(jsheetString)
    do {
      splitJcss = this.split(jsheetString)
      parsedCss += (buffer = this.interpret(splitJcss))
      if(buffer === false) return false
    } while (jsheetString = splitJcss[2])
    return parsedCss
  }
}
